...
class CheckoutController extends Controller
{
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        foreach ($carts as $key => $cartItem) {
            ...
            $weight += (int)$cartItem->product->weight;
        }
        ...
        $rajaongkir = self::rajaongkir($request->address_id, $weight);

        return view('frontend.delivery_info', compact('carts', 'carrier_list', 'rajaongkir'));
    }
            ...
            foreach ($carts as $key => $cartItem) {
                ...
                if (get_setting('shipping_type') != 'carrier_wise_shipping' || $request['shipping_type_' . $product->user_id] == 'pickup_point') {
                    if ($request['shipping_type_' . $product->user_id] == 'pickup_point') {
                        ...
                        $cartItem['shipping_type'] = $request->selected_services;
                    }
                    $cartItem['shipping_cost'] = 0;
                    if ($cartItem['shipping_type'] == $request->selected_services) {
                        $cartItem['shipping_cost'] = $request->shipping; //getShippingCost($carts, $key);
                    }
                } else {
                    $cartItem['shipping_type'] = 'carrier';
                    $cartItem['carrier_id'] = $request['carrier_id_' . $product->user_id];
                    $cartItem['shipping_cost'] = $request->shipping; //getShippingCost($carts, $key, $cartItem['carrier_id']);
                }

                $shipping += $cartItem['shipping_cost'];
                $cartItem->save();
            }
            $total = $subtotal + $tax + (int)$request->shipping;
    ...
    public function rajaongkir($address_id, $weight)
    {
        ...
        $origin = 419; // Replace with your actual origin
        $destination = $address->city_id; // Replace with your actual destination
        $weight = $weight; // Replace with your actual weight
