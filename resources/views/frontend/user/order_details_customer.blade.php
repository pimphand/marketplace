...
<div class="row">
    ...
    <div class="col-md-3">
        ...
        @if ($order->manual_payment && $order->manual_payment_data == null || $order->payment_type == 'duitku')
        <button onclick="show_make_payment_modal({{ $order->id }})" class="btn btn-block btn-success">
            {{ translate('Make Payment') }}</button>
        @endif
    </div>
</div>
@endsection
...
<script type="text/javascript">
    ...
    let feePayment = 0;
    function paymentMethod(data,fee) {
        paymentMethods = data;
        feePayment = fee;
    }
    $(document).ready(function () {
        if ($('#payment_modal_body').length > 0) {
            $('#payment_modal_body').on('click', '#_button-duitku', function (e) {
                e.preventDefault();
                $.post('{{ route('duitkuInquiry') }}', {
                    _token: '{{ csrf_token() }}',
                    order_id: order_ids,
            ...
            console.error('#payment_modal_body not found in the DOM.');
        }
    });
